...
Cálculo de tasas de empleo por rama (Industria vs Turismo)
y generación de gráficos por aglomerado y año.
...
Turismo:
    - Hoteles y restaurantes: 5600–5999
    - Actividades artísticas / de entretenimiento: 9100–9499
    - Transporte y actividades conexas: 5000–5599
      (tomado completo como "sector transporte" dentro del complejo turístico)

La tasa de empleo por rama (TE_RAMA) se define como:

    TE_RAMA(aglo, año, rama) =
        sum(PONDERA de ocupados de esa rama, en ese aglo y año, edad >= 18)
        -------------------------------------------------------------------
        sum(PONDERA de toda la población 18+ de ese aglo y año)

Salida principal:
    - DataFrame con columnas:
        AGLOMERADO, ANO4, RAMA, TE_RAMA
    - Gráficos de barras anuales por aglomerado.
"""
...
# ---------------------------------------------------------
# Cálculo de tasas de empleo por rama
# ---------------------------------------------------------

def tasas_empleo_por_rama(df: pd.DataFrame,
                          aglos_target=None,
                          edad_min: int = 18) -> pd.DataFrame:
    """
    Calcula la tasa de empleo anual por rama (Industria / Turismo)
    ...
    Parámetros:
        ...
        edad_min : edad mínima para considerar en la población (default 18)
    ...
    df = agregar_columna_rama(df)

    # -------------------------------------------------
    # Denominador: población total 18+ por aglo y año
    # -------------------------------------------------
    poblacion = (
        df.groupby(["AGLOMERADO", "ANO4"], as_index=False)["PONDERA"]
          .sum()
          .rename(columns={"PONDERA": "POBL_18MAS"})
    )

    # -------------------------------------------------
    # Numerador: ocupados por rama, aglo y año
    # -------------------------------------------------
    # Filtramos solo personas con rama conocida y ocupadas (ESTADO=1)
    df_ocup_rama = df[(df["RAMA_TUR"].notna()) & (df["ESTADO"] == 1)].copy()
    ...
    merged["TE_RAMA"] = merged["OCUPADOS_RAMA"] / merged["POBL_18MAS"]

    merged = merged.rename(columns={"RAMA_TUR": "RAMA"})

    # Orden prolijo
    merged = merged[["AGLOMERADO", "ANO4", "RAMA", "OCUPADOS_RAMA", "POBL_18MAS", "TE_RAMA"]]

    return merged


# ---------------------------------------------------------
# Exportar tabla de tasas de empleo por rama
# ---------------------------------------------------------

def exportar_tasas_rama(df_tasas: pd.DataFrame,
                        carpeta_salida: str = "output/tasas") -> tuple[str, str]:
    """
    Exporta la tabla de tasas por rama a CSV + XLSX con versionado.
    ...
    csv_path = os.path.join(carpeta_salida, "tasas_empleo_ramas.csv")
    xlsx_path = os.path.join(carpeta_salida, "tasas_empleo_ramas.xlsx")

    csv_path = versionar_archivo(csv_path)
    xlsx_path = versionar_archivo(xlsx_path)

    df_tasas.to_csv(csv_path, index=False)
    df_tasas.to_excel(xlsx_path, index=False)
...
def graficar_tasas_rama(df_tasas: pd.DataFrame,
    ...
    # Aglomerados presentes
    for aglo, df_aglo in df_tasas.groupby("AGLOMERADO"):
        ...
        ax.set_title(f"Tasa de empleo por rama - Aglomerado {aglo}")
        ax.set_xlabel("Año")
        ...
        fname = os.path.join(carpeta_salida, f"te_ramas_aglo_{aglo}.png")
...
if __name__ == "__main__":
    ...
    if os.path.exists(eph_path):
        ...
        # Solo aglos 31 y 34
        df_t = tasas_empleo_por_rama(df_eph, aglos_target=[31, 34])

        csv, xlsx = exportar_tasas_rama(df_t)
        print("Tasas por rama exportadas en:")
        print("  CSV :", csv)
        print("  XLSX:", xlsx)

        paths_png = graficar_tasas_rama(df_t)
